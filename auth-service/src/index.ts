  ...
  async (req: any, reply) => {
    ...
    try {
      const res = await fetch(
        `${PROFILE_SERVICE_URL}/internal/profile/by-user-id/${user.id}`,
        {
...
fastify.get<{ Params: { id: string } }>(
  "/internal/auth/session_state/:id",
      ...
      >((resolve, reject) => {
        db.get(
          ...
          (err, result) => {
            ...
            resolve(
              result as { user_id: string; expires_at: string } | undefined,
            );
          },
        );
      });
  ...
  async (req: any, reply) => {
    ...
    try {
      ...
      reply
        .status(409)
        .send({
          error: { code: "EMAIL_OR_USERNAME_TAKEN", message: "Already exists" },
        });
    }
  },
);
...
fastify.post(
  ...
  async (req: any, reply) => {
    ...
    const { old_password, new_password }: AuthInterfaces.ChangePassword =
      result.data;
    ...
    const user = await new Promise<AuthInterfaces.DBUser | null>(
      (resolve, reject) => {
        db.get(
          `SELECT password_hashed FROM users WHERE id = ?`,
          [userId],
          (err, row) =>
            err ? reject(err) : resolve(row as AuthInterfaces.DBUser | null),
        );
      },
    );
...
fastify.post("/auth/logout", async (req: any, reply) => {
  ...
  try {
    ...
    const res = await fetch(
      `${PROFILE_SERVICE_URL}/internal/profile/logout/${user.id}`,
      {
        headers: { Authorization: `Bearer ${SERVICE_TOKEN}` },
        signal: controller.signal,
      },
  ...
  async (req: any, reply) => {
    ...
    try {
      ...
      const profileRes = await fetch(
        `${PROFILE_SERVICE_URL}/internal/profile/delete`,
        {
          method: "POST",
          headers: {
      ...
      const res = await fetch(
        `${PROFILE_SERVICE_URL}/internal/profile/logout/${userId}`,
        {
          headers: { Authorization: `Bearer ${SERVICE_TOKEN}` },
          signal: controller.signal,
        },
...
const start = async () => {
  try {
    ...
    console.log(
      chalk.green.bold("[auth] Authentification is running on :8081"),
    );
  } catch (err) {
