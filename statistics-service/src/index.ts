...
import {
  leaderboardLoop,
  getLeaderboardByIndex,
} from "./workers/leaderboardWorker";
...
import {
  ...
  getStatisticsDB,
  closeStatisticsDB,
} from "./database/dbStats";
import {
  initLeaderboardDB,
  getLeaderboardDB,
  closeLeaderboardDB,
} from "./database/dbLeaderboard";
...
// --- ADD GAME STATISTICS ---
fastify.post<{ Body: StatsTypes.GameResult }>(
  "/internal/statistics/gameresult/update",
  { preHandler: requireServiceAuth, schema: gameResultSchema },
  async (req: any, reply) => {
    try {
      const { game_id, start_at, end_at, players } = req.body;
...
// --- GET LEADERBOARD ---
fastify.get<{ Querystring: StatsTypes.LeaderboardQuery }>(
  ...
  async (req, reply) => {
    try {
      ...
      const data = await getLeaderboardByIndex(by as any, limit, offset);
...
fastify.get(
  "/internal/statistics/games/history/:id",
...
async function start() {
  ...
  try {
    ...
    // --- WAIT WORKERS ---
    await Promise.allSettled([statsWorker, leaderboardWorker]);
