      	...
      	return new Promise((resolve, reject) => {
	    	db.serialize(async () => {
		  	try {
				...
				await run(`CREATE INDEX IF NOT EXISTS idx_games_processed_processing ON games_and_results(processed, processing)`);
				await run(`CREATE INDEX IF NOT EXISTS idx_games_u1 ON games_and_results(user1_id)`);
				await run(`CREATE INDEX IF NOT EXISTS idx_games_u2 ON games_and_results(user2_id)`);
