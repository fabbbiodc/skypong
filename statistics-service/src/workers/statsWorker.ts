...
async function lockGames(): Promise<StatsTypes.GameRow[]> {
  ...
  try {
    ...
    const games = await all<StatsTypes.GameRow>(
      `SELECT * FROM games_and_results WHERE processed = 0 AND processing = 0 LIMIT ?`,
      [BATCH_SIZE],
    );
    ...
    await run(
      `UPDATE games_and_results SET processing = 1 WHERE game_id IN (${placeholders})`,
    );
...
async function processGame(game: StatsTypes.GameRow) {
  ...
  await axios.post(
    `${PROFILE_API}/internal/profile/gameresult/update`,
    payload,
    {
      timeout: 5000,
      headers: {
  ...
  await db.run(
    `UPDATE games_and_results SET processed = 1, processing = 0, processed_at = CURRENT_TIMESTAMP WHERE game_id = ?`,
    [game.game_id],
  );
}
...
async function unlockGame(gameId: string) {
  await db.run(
    `UPDATE games_and_results SET processing = 0 WHERE game_id = ?`,
    [gameId],
  );
}

// --- Worker loop ---
export async function statisticsLoop(
