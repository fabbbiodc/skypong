...
export async function acceptFriendRequestService(
  ...
  if (status.blocked_by === userId || status.blocked_by === requesterId)
    throw new Error("CANNOT_ACCEPT_BLOCKED");
...
export async function cancelFriendRequestService(
  ...
  const status = await repo.getFriendStatus(userId, requesterId);
  if (!status || status.status !== "pending" || status.requester_id !== userId)
    throw new Error("REQUEST_NOT_FOUND");
...
export async function unblockUserService(
  ...
  if (!status || status.status !== "blocked" || status.blocked_by !== userId)
...
export async function getFriendsOfTargetService(
  userId: string,
  targetId: string,
): Promise<ProfileTypes.FriendUser[]> {
...
export async function getIncomingRequestsService(
  userId: string,
): Promise<ProfileTypes.FriendUser[]> {
...
export async function getOutgoingRequestsService(
  userId: string,
): Promise<ProfileTypes.FriendUser[]> {
...
// --- GET BLOCK LIST ---
export async function getBlocklistService(
  userId: string,
): Promise<ProfileTypes.FriendUser[]> {
...
// --- GET FRIEND STATUS ---
export async function getFriendStatusService(
  userId: string,
  otherId: string,
