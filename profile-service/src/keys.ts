...
function readPublicKeyOrExit(): string {
  const publicKeyPath = resolveKeyPath(
  ...
  while (!fs.existsSync(publicKeyPath) && attempts < 10) {
    console.log(
      `[profile] Attempt ${attempts} Waiting for public key at ${publicKeyPath}...`,
    );
  ...
  if (fs.existsSync(publicKeyPath)) {
    console.log(
      `[profile] JWT public key loaded from ${publicKeyPath} after ${attempts} attempt(s)`,
    );
  }
