...
export const playerDataSchema = (t: any) => {
  ...
  const errors = {
    ...
    nicknameMinLength: t?.user?.errors?.nicknameMinLength(4) || 'Nickname must be at least 8 characters',
    whinphraseRequired: t?.user?.errors?.winphraseRequired || 'Winphrase is required',
    winphraseMinLength: t?.user?.errors?.winphraseMinLength(4) || 'Winphrase must be at least 8 characters',
  };
