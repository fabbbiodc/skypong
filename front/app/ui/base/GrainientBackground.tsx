...
function applyPostProcessing(
  ...
  for (let y = 0; y < height; y += grainStep) {
    for (let x = 0; x < width; x += grainStep) {
      ...
      const noiseValue = (simpleNoise(
        x * options.noiseScale * 0.01,
        y * options.noiseScale * 0.01,
        noiseSeed
      ) - 0.5) * 255 * options.grainAmount;
...
export function GrainientBackground({
  ...
  useEffect(() => {
    ...
    const getColors = () => {
      ...
      return [
        hexToRgb(styles.getPropertyValue('--color-bg-gradient-1').trim() || '#9333ea'),
        hexToRgb(styles.getPropertyValue('--color-bg-gradient-2').trim() || '#ffffff'),
        hexToRgb(styles.getPropertyValue('--color-bg-gradient-3').trim() || '#B19EEF'),
      ];
    };
    ...
    const animate = () => {
      ...
      const angle = (time * rotationAmount * 0.001) + (blendAngle * Math.PI / 180);
      const warpOffsetX = Math.sin(time * warpSpeed * warpFrequency) * warpAmplitude * warpStrength;
      const warpOffsetY = Math.cos(time * warpSpeed * warpFrequency * 0.7) * warpAmplitude * warpStrength;
      
      const centerOffsetX = centerX * canvas.width * 0.5;
      const centerOffsetY = centerY * canvas.height * 0.5;
      
      const gradient = ctx.createLinearGradient(
        canvas.width * 0.5 + centerOffsetX + warpOffsetX + Math.cos(angle) * canvas.width * zoom,
        canvas.height * 0.5 + centerOffsetY + warpOffsetY + Math.sin(angle) * canvas.height * zoom,
        canvas.width * 0.5 + centerOffsetX - warpOffsetX - Math.cos(angle) * canvas.width * zoom,
        canvas.height * 0.5 + centerOffsetY - warpOffsetY - Math.sin(angle) * canvas.height * zoom
      );
      
      // 2. Calculate animated color stops with color balance
      const colorMix1 = interpolateColors(colors[0], colors[1], 
        Math.abs(Math.sin(time * 0.3)) * (1 + colorBalance));
      const colorMix2 = interpolateColors(colors[1], colors[2], 
        Math.abs(Math.cos(time * 0.5)) * (1 - colorBalance));
      const colorMix3 = interpolateColors(colors[2], colors[0], 
