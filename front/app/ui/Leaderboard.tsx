    ...
    >
      ...
      <div className="flex flex-1 min-w-0 flex-col gap-0.5">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge
            ...
            variant={
              playerStatus === PLAYER_STATUS.active
                ? "success"
                : playerStatus === PLAYER_STATUS.absent
                  ? "warning"
                  : playerStatus === PLAYER_STATUS.blocked
                    ? "danger"
                    : "neutral"
            }
            shape="pill"
          >
            {playerStatus === PLAYER_STATUS.absent
              ? t.player.absent || "Absent"
              : ""}
            {playerStatus === PLAYER_STATUS.inactive
              ? t.player.inactive || "Inactive"
              : ""}
            {playerStatus === PLAYER_STATUS.active
              ? t.player.active || "Active"
              : ""}
            {playerStatus === PLAYER_STATUS.blocked
              ? t.player.blocked || "Blocked"
              : ""}
          </Badge>
        </div>
        ...
        <span className="text-xs text-muted">
          {t.player.winRate}: {Math.round(player.winrate * 100) + "%"}
        </span>
      </div>
...
export default function Leaderboard({ userId }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {error && (
        <p className="text-danger text-sm text-center py-8">Error: {error}</p>
      )}
      ...
      {players.map((p, i) => (
        <LeaderboardRow
          ...
          player={p}
          rank={i + 1}
          profileURL={whoisURL(p.user_id, user?.id)}
          isMe={isMe(p.user_id, user?.id)}
        />
      ))}
    </div>
  );
