...
function DropdownMenu({ items, onClose }) {
  ...
  return (
    <div
      ...
      className="absolute right-0 top-full z-20 mt-2 w-44 rounded-lg border border-slate-700 bg-transparent p-1 shadow-lg"
    >
      {items.map((item) => (
        <button
          key={item.label}
...
export default function AddFriendButton({
  ...
  const getStateStyles = () => ({
    ...
    blocked_by: {
      ...
      classes: "bg-slate-700/30 text-slate-200 cursor-not-allowed",
    },
  });
  ...
  if (relation === undefined) {
    return (
      <div className="font-display text-[11px] text-slate-200 tracking-wide">
        ...
      </div>
    );
  }
