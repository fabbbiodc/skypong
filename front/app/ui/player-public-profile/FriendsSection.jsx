...
import {
  checkPlayerStatus,
  PLAYER_STATUS,
} from "../../lib/players/check-player-status";
...
function Empty({ icon, text }) {
  return (
    ...
    >
      <div style={{ fontSize: "30px", marginBottom: "14px", opacity: 0.35 }}>
        {icon}
      </div>
      {text}
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ◈ {t?.player?.incomingReqest || "INCOMING REQUEST"}
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ◌ {t?.player?.pendingResponse || "PENDING RESPONSE"}
        </div>
      </div>
      <Pill
...
export default function FriendsSection({
  ...
  const handleReject = (id) =>
    act(
      () => api(`/api/profile/friends/${id}/reject`, { method: "POST" }),
      "Solicitud rechazada",
    );
  const handleCancel = (id) =>
    act(
      () => api(`/api/profile/friends/${id}/cancel`, { method: "POST" }),
      "Solicitud cancelada",
    );
  ...
  const inactiveFriends = friends.filter(
    (f) => (isAbsent(f.last_login) || !f.logged) && !isBlocked(f.status),
  );
  ...
  const tabs = [
    {
      key: "friends",
      label: `${t?.player?.friends || "Friends"}`,
      count: friends.length,
    },
    {
      key: "incoming",
      label: `${t?.player?.incoming || "Incoming"}`,
      count: incoming.length,
    },
    {
      key: "outgoing",
      label: `${t?.player?.outgoing || "Outgoing"}`,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          {tab === "friends" &&
            (friends.length === 0 ? (
              <Empty
                icon="👾"
                text={t?.player?.younofriends || "You still have no friends"}
              />
            ) : (
              <>
                {activeFriends.length > 0 && (
                  <>
                    <SectionLabel
                      label={`${t?.player?.actives || "ACTIVES"} — ${activeFriends.length}`}
                      color={C.green}
                    />
                    {activeFriends.map((f) => (
                      <FriendRow
                ...
                {inactiveFriends.length > 0 && (
                  <>
                    <SectionLabel
                      label={`${t?.player?.inactives || "INACTIVES"} — ${inactiveFriends.length}`}
                      color={C.textDim}
                    />
                ...
                {blockedFriends.length > 0 && (
                  <>
                    <SectionLabel
                      label={`${t?.player?.bloqueds || "BLOCKED"} — ${blockedFriends.length}`}
                      color={C.textDimRed}
                    />
          ...
          {tab === "incoming" &&
            (incoming.length === 0 ? (
              <Empty
                icon="📭"
                text={t?.player.noincomingRequests || "NO INCOMING REQUESTS"}
              />
            ) : (
              incoming.map((r) => (
                <IncomingRow
          ...
          {tab === "outgoing" &&
            (outgoing.length === 0 ? (
              <Empty
                icon="📤"
                text={t?.player?.nooutgoingRequests || "NO OUTGOING REQUESTS"}
              />
            ) : (
              outgoing.map((r) => (
                <OutgoingRow
