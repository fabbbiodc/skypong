...
export class AIPaddleController {
    ...
    private updateDegradation(): void {
        const newLevel = Math.min(
            Math.floor(this.rallyTickCount / this.settings.degradationTickInterval) + Math.floor(this.accumulatedDegradation),
    ...
    private getEffectiveAccuracy(): number {
        const degraded = this.settings.accuracy - (this.degradationLevel * AI_BEHAVIOR.DEGRADATION.ACCURACY_PENALTY_PER_LEVEL);
        return Math.max(AI_BEHAVIOR.DEGRADATION.MIN_ACCURACY, Math.min(this.settings.accuracy, degraded));
    }
    ...
    private getEffectiveErrorFrequency(): number {
        const degraded = this.settings.errorFrequency * (1 + this.degradationLevel * AI_BEHAVIOR.DEGRADATION.ERROR_FREQ_BOOST_PER_LEVEL);
    ...
    public partialReset(): void {
        ...
        const totalDegradation = this.accumulatedDegradation + this.degradationLevel;
        this.accumulatedDegradation = Math.max(0, totalDegradation * AI_BEHAVIOR.DEGRADATION.RESET_CARRY_FACTOR);
        this.degradationLevel = Math.max(0, Math.floor(this.accumulatedDegradation));
    }
