...
export class PhysicsEngine {
    ...
    public updateBall(body: BallBody, deltaTimeMs: number): void {
        ...
        if (
            ...
            Logger.physics(`Ball marked as falling: z=${body.mesh.position.z.toFixed(2)}, threshold=${ROOM_CONFIG.FALL_THRESHOLD_Z}`);
            body.isInFall = true;
        }

        // Check if ball fell below threshold
        if (body.isInFall && body.mesh.position.y <= body.fallThreshold) {
            Logger.physics(`Ball disabled: y=${body.mesh.position.y.toFixed(2)}, fallThreshold=${body.fallThreshold}`);
     ...
     */
    public resolveBallBorderCollision(ball: BallBody, collisionResult: BorderCollisionResult): void {
    ...
    public resolveBallPaddleCollision(
        ...
        const offsetFromCenter = Math.max(-1, Math.min(1, (impactX - paddleCenterX) / paddleHalfWidth));

        // Bounce angle: 0 at center, up to MAX_BOUNCE_ANGLE_DEG degrees at edges (in radians)
        const maxBounceAngle = (PHYSICS.RESPONSE.MAX_BOUNCE_ANGLE_DEG * Math.PI) / 180;
     ...
     */
    public launchBall(body: BallBody, directionX?: number, directionZ?: number): void {
        body.isInFall = false;
        ...
        if (directionX === undefined || directionZ === undefined) {
            ...
            const angle = Math.random() * (PHYSICS.LAUNCH.MAX_ANGLE_DEG - PHYSICS.LAUNCH.MIN_ANGLE_DEG)
     ...
     */
    public createBallBody(mesh: Mesh, tableWidth: number, tableDepth: number): BallBody {
        const spawnY =
