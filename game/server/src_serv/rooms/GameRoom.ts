...
export class GameRoom extends Room<MyGameState> {
  ...
  update(deltaTime: number) {
    ...
    if (
      ...
      if (Math.abs(ballZ) < ROOM_CONFIG.GOAL_THRESHOLD) {
        ...
        Logger.debug(
          `Goal flag reset - ball back in play at z=${ballZ.toFixed(2)}`,
        );
      }
    }
  ...
  private checkForGoal(): void {
    ...
    if (Math.abs(ballZ) > 4.5) {
      Logger.debug(
        `checkForGoal: ballZ=${ballZ.toFixed(2)}, gameOver=${this.state.gameOver}, ballLaunched=${this.ballLaunched}, ballEnabled=${this.serverBall.isEnabled()}, goalScored=${this.goalScoredThisRound}, threshold=${ROOM_CONFIG.GOAL_THRESHOLD}`,
      );
    }
    ...
    if (ballZ > ROOM_CONFIG.GOAL_THRESHOLD) {
      Logger.goal(
        `Player 1 scores! ballZ=${ballZ.toFixed(2)} > threshold=${ROOM_CONFIG.GOAL_THRESHOLD}`,
      );
    ...
    if (ballZ < -ROOM_CONFIG.GOAL_THRESHOLD) {
      Logger.goal(
        `Player 2 scores! ballZ=${ballZ.toFixed(2)} < -threshold=${-ROOM_CONFIG.GOAL_THRESHOLD}`,
      );
      this.goalScoredThisRound = true;
      this.awardPoint(2);
      return;
  ...
  private awardPoint(playerNum: number): void {
    if (playerNum === 1) {
      ...
      Logger.info(
        `Goal! Player 1 (${this.state.player1Name}) scores! Score: ${this.state.player1Score}-${this.state.player2Score}`,
      );
    } else {
      this.state.player2Score++;
      Logger.info(
        `Goal! Player 2 (${this.state.player2Name}) scores! Score: ${this.state.player1Score}-${this.state.player2Score}`,
      );
    }
  ...
  private checkForWinner(): void {
    ...
    if (this.state.player1Score >= winningScore) {
      ...
      Logger.gameOver(
        `Winner: ${this.state.player1Name} (${this.state.player1Score}-${this.state.player2Score})`,
      );
      ...
      this.serverBall.setEnabled(false);
      Logger.gameOver(
        `Winner: ${this.state.player2Name} (${this.state.player1Score}-${this.state.player2Score})`,
      );
    }
  }
  ...
  onJoin(client: Client, options: any): void | Promise<any> {
    ...
    if (!this.player1Client) {
      ...
      Logger.info(
        `Player 1 joined: ${this.state.player1Id} (${this.state.player1Name}, color: ${this.state.player1Color})`,
      );
      ...
      if (isLocal2P) {
        ...
        Logger.info(
          `Local Player 2 configured: ${this.state.player2Name} (${this.state.player2Id}), color: ${this.state.player2Color}`,
        );
      ...
      Logger.info(
        `Player 2 joined: ${this.state.player2Id} (${this.state.player2Name}, color: ${this.state.player2Color})`,
      );
