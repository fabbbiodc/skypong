...
export interface RoomManagerCallbacks {
    ...
    onScoreUpdate?: (params: { player1Score: number; player2Score: number }) => void;
    onGameOver?: (params: {
        winner: string;
        player1Name: string;
        player2Name: string;
    ...
    onPlayerNameUpdate?: (params: { player1Name: string; player2Name: string }) => void;
    onGameStarted?: () => void;
    onRoomExpired?: () => void;
...
export class RoomManager {
    ...
    get isOnlineMode(): boolean {
        return this._gameMode === 'online-create' || this._gameMode === 'online-join';
    }
    ...
    async connect(
        ...
        switch (gameMode) {
            ...
            case 'ai-medium':
            case 'ai-hard':
                room = await client.create<GameState>(SERVER_CONNECTION.ROOMS.AI_GAME_ROOM, {
                    ...joinOptions,
