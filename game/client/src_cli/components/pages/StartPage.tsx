        ...
        <div style={STYLES.colorPicker}>
            {PLAYER_COLORS.map((color) => (
                <button
                    ...
                    title={color.name}
                    style={{ ...STYLES.colorButton(selectedColor === color.hex), backgroundColor: color.hex }}
                />
            ))}
        </div>
    </div>
...
const StartPage = () => {
    ...
    const fetchRooms = useCallback(async () => {
        try {
            ...
            const openRooms = rooms.filter((r: any) => r.metadata?.waitingForOpponent && r.clients < 2);
    ...
    if (isOnlineMode && lobbyPhase === 'lobby') {
        return (
            <div style={STYLES.container}>
                ...
                <p style={{ color: '#aaa', fontSize: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    Playing as:
                    <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <div style={{ width: '14px', height: '14px', borderRadius: '50%', backgroundColor: player1Color, border: '2px solid #666' }} />
                        <span style={{ color: player1Color, fontWeight: 'bold' }}>{player1Name}</span>
                    </span>
                </p>
                {lobbyError && (
                    <div style={{ padding: '10px 20px', backgroundColor: '#f4433622', border: '1px solid #f44336', borderRadius: '8px', color: '#f44336', fontSize: '14px', maxWidth: '350px', textAlign: 'center' }}>
                        {lobbyError}
                    </div>
                )}
                <div style={STYLES.roomList}>
                    {availableRooms.length === 0 ? (
                        <div style={{ padding: '20px', backgroundColor: '#2d2d44', borderRadius: '8px', color: '#888', textAlign: 'center' }}>
                        ...
                        availableRooms.map((room) => (
                            <div key={room.roomId} style={STYLES.roomItem}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    <div style={{ width: '16px', height: '16px', borderRadius: '50%', backgroundColor: room.metadata?.player1Color || '#00A6ED', border: '2px solid #666' }} />
                                    <span style={{ color: 'white', fontSize: '16px' }}>{room.metadata?.player1Name || 'Unknown'}</span>
                                </div>
                                <button
                                    onClick={() => handleJoinRoom(room.roomId)}
                                    disabled={lobbyLoading}
                                    style={{ padding: '8px 20px', fontSize: '14px', backgroundColor: lobbyLoading ? '#666' : '#4CAF50', color: 'white', border: 'none', borderRadius: '6px', cursor: lobbyLoading ? 'not-allowed' : 'pointer', fontWeight: 'bold' }}
                                >
                                    Join
                                </button>
                            </div>
                ...
                <div style={{ display: 'flex', gap: '12px', marginTop: '10px', width: '350px' }}>
                    <button onClick={handleBackToMenu} style={STYLES.backButton}>Back</button>
                    <GameButton onClick={handleCreateRoom} color={lobbyLoading ? '#666' : '#9C27B0'} hoverColor='rgba(156, 39, 176, 0.4)'>Create Room</GameButton>
                    <button onClick={fetchRooms} style={{ padding: '15px 15px', fontSize: '16px', backgroundColor: '#333', color: '#aaa', border: '1px solid #555', borderRadius: '8px', cursor: 'pointer' }} title='Refresh room list'>Refresh</button>
                </div>
            </div>
        );
    }
    ...
    if (selectedMode && !isOnlineMode) {
        return (
            <div style={STYLES.container}>
                ...
                <div style={STYLES.formContainer}>
                    <div style={{ width: '100%' }}>
                        <label style={STYLES.label}>{isAiMode ? 'Your Name' : 'Player 1 Name'}</label>
                        <input
                            type='text'
                            value={player1Name}
                            onChange={(e) => setPlayer1Name(e.target.value)}
                    ...
                    <ColorPicker selectedColor={player1Color} onColorSelect={setPlayer1Color} label={isAiMode ? 'Choose Your Color' : 'Player 1 Color'} />
                    {selectedMode === '2p-local' && (
                        <>
                            ...
                            <ColorPicker selectedColor={player2Color} onColorSelect={setPlayer2Color} label='Player 2 Color' />
                        </>
                    )}
                    {isAiMode && (
                        <div style={{ width: '100%', padding: '12px 16px', backgroundColor: '#2d2d44', borderRadius: '8px', color: '#888', textAlign: 'center' }}>
    ...
    return (
        <div style={STYLES.container}>
            ...
            <div style={STYLES.section}>
                <p style={{ color: '#aaa', marginBottom: '10px', fontSize: '18px' }}>Single Player vs AI</p>
                <GameButton onClick={() => handleModeSelect('ai-easy')} color='#4CAF50' hoverColor='rgba(76, 175, 80, 0.4)'>Easy</GameButton>
                <GameButton onClick={() => handleModeSelect('ai-medium')} color='#FF9800' hoverColor='rgba(255, 152, 0, 0.4)'>Medium</GameButton>
                <GameButton onClick={() => handleModeSelect('ai-hard')} color='#f44336' hoverColor='rgba(244, 67, 54, 0.4)'>Hard</GameButton>
            </div>
            <div style={{ ...STYLES.section, ...STYLES.sectionDivider }}>
                <p style={{ color: '#aaa', marginBottom: '10px', fontSize: '18px' }}>Multiplayer</p>
                <GameButton onClick={() => handleModeSelect('2p-online')} color='#9C27B0' hoverColor='rgba(156, 39, 176, 0.4)'>Online PvP</GameButton>
                <GameButton onClick={() => handleModeSelect('2p-local')} color='#2196F3' hoverColor='rgba(33, 150, 243, 0.4)'>2 Player Local</GameButton>
            </div>
