    ...
    const tex = new Texture(
      ...
      () => {
        if (tex.url && tex.url.endsWith(".jpg")) {
          console.warn(`Fallback: ${suffix} not found as JPG, trying PNG.`);
  ...
  public static CreatePBRMaterial(
    ...
    if (options?.isRefractive) {
      ...
      material.subSurface.refractionIntensity =
        options.refractionIntensity ?? 1.0;
      ...
      material.subSurface.tintColor =
        options.tintColor ?? new Color3(0.95, 0.95, 0.95);
      material.subSurface.tintColorAtDistance =
        options.tintColorAtDistance ?? 1.0;
